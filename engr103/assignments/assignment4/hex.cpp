...
int main() { // 11 lines
	...
	number =  g_c_d(numerator, denominator);
...
bool to_lowest_terms(int &numerator, int &denominator){ 
	...
	numerator = numerator/g_c_d(numerator, denominator); // divides numerator to lowest terms
	denominator = denominator/g_c_d(numerator, denominator); // divides denominator to lowest terms
}

int g_c_d(int numerator, int denominator){
	int divisor = 1; 
	for(int i = 2; i <= 7; i++) {
