...
** Author: Bruce Yan
** Date: 1/22/2023
** Description: linked list data base 
** Input: test inputs 
** Output: test ouputs
******************************************************/
 /*
...
{
    //allocate memory in the heap for list
...
{
    //create a current node for visual purposes
    //assign head pointer to current
    struct node* current = list->head;
    //create a temp node
    struct node* temp = NULL;
    //traverse through the list
    while(current != NULL){
        //temp gets the pointer to the next node in list
        temp = current->next;
        //free the current node
        free(current);
        //assign current to the temp to continue through the list
        current = temp; 
    }
    //set head to NULL
    list->head = NULL;
...
{
    //create a new node and assign pointer to it
    struct node* current = malloc(sizeof(struct node));
    //set the value in new node
    current->val = val;
    //set the node to point to beginning of the list
    current->next = list->head;
    //set the beginning of the list to the newly created node
    list->head = current;
    return;
}
...
{
    //assigns current the head of the list
    struct node* current = list->head;
    if (list->head == NULL){
        struct node* current = malloc(sizeof(struct node));
        //set the value in new node
    ...
    //traverse through the list
    while(current != NULL){
        //condition signals being at back of the list
        if (current->next == NULL){
            //create new node
            current->next = malloc(sizeof(struct node));
            //assign current 
            current = current->next;
            //assign current->next NULL;
            current->next = NULL;
            //assign inputer val
            current->val = val;
        }        
...
void list_remove(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    //condition for if we want to remove the first node
    if(current != NULL && cmp(val,current->val) == 0){
        //set the next node in list as the head
    ...
    //traverse through the list
    while(current != NULL){
        //condition, if the value we are looking for is in the list
        if (cmp(val, current->val) == 0){
            //set pointer of our previous node to next node
            //connects the list back together 
...
int list_position(struct list* list, void* val, int (*cmp)(void* a, void* b))
{
    ...
    //traverse through the list
    while(current != NULL){
        //if we find the val we want in the list
        if (cmp(val, current->val) == 0){
            //return the position
            return ctr;
        }
    ...
    //return if val we are looking for is not in the list
    return -1;
}
...
{   
    ...
    //traverse through the list
    while(current != NULL){
        //we save the next node in the list to be the next node
        next = current->next;
        //our next node gets our previous
        current->next = previous;
        //our previous is now our current
        previous = current; 
        //our current now is our next;
        current = next;
    }
    list->head = previous;
