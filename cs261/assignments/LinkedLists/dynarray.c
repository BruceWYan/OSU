...
** Author: Bruce Yan
** Date: 1/22/2023
** Description: dynamic array database 
** Input: test inputs 
** Output: test ouputs
******************************************************/
...
{
    //create space in the heap for struct
    struct dynarray *da = malloc(sizeof(struct dynarray)); 
    //set size to 0
    da->size = 0;
    //set capacity to 2
    da->capacity = 2;
    //allocate space in the heap for array of void pointers
    da->data = malloc(da->capacity * sizeof(void*));
    //return pointer to the struct in the heap
    return da;
}
...
{
    //if the array is not empty we can delete
    if (da->data != NULL)
        free(da->data); 
    //if the stuct is not empty we also delete it
    if (da != NULL)
        free(da);
    return;
}
...
{   
    int i;
    //if we don't have any more space in array
    if (da->size == da->capacity){
        //double capacity
        da->capacity*=2;
        //create a temporary array of new size
        void** temp_data = malloc(da->capacity * sizeof(void*));
        //deep copy
        for(i = 0; i < da->size; i++){ 
            temp_data[i] = da->data[i];
        }
        //delete old array
        free(da->data);
...
void dynarray_remove(struct dynarray* da, int idx)
{
    ...
    for(i = 0; i < da->size; i++){
        //if we find the index we are looking for
        if (idx == i){
...
void* dynarray_get(struct dynarray* da, int idx)
{
    ...
    for (i = 0; i < da->size; i++){
        //condition for getting the pointer we want
        if (idx == i)
            //return the pointer
            return da->data[i];
    }
