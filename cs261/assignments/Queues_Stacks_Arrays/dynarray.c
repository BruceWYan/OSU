...
void dynarray_insert(struct dynarray* da, void* val) {
   ...
   */
  int physical = (da->size + da->start) % da->capacity;
...
/** Function: dynarray_front
** Description: shows front of array
** Parameters: struct dynarray* da 
** Pre-Conditions: da must exist 
** Post-Conditions: returns void pointer
**/
void* dynarray_front(struct dynarray* da){
  return da->data[da->start];
}

/** Function: dynarray_reindex
** Description: decreases the size and moves start up by 1 as if to remove an item
** Parameters: struct dynarray* da
** Pre-Conditions: da must exist
** Post-Conditions: manipulates start and size
**/
void dynarray_reindex(struct dynarray* da){
  da->start = (da->start + 1) % da->capacity;
  da->size--;
  //increases start, if the start is equal to the capcity at any point
  //we will resize the the array and then reset da->start back to 0 
}


/** Function: dynarray_isempty
