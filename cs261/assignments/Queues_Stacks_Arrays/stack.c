...
int stack_isempty(struct stack* stack) {
	int empty = list_isempty(stack->list);
	if (empty == 1)
		return 1;
	else
		return 0;
...
void* stack_top(struct stack* stack) {
	void* top_val = get_head_val(stack->list);
	return top_val;
}
...
void* stack_pop(struct stack* stack) {
	void* pop_val = get_head_val(stack->list);
	list_remove(stack->list, pop_val, &compare_values);
	return pop_val;

}

/** Function: stack_size
** Description: returns the size of the list in the stack
** Parameters: struct stack* stack
** Pre-Conditions: stack has to exist
