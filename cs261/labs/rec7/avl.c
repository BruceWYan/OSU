...
struct avl_node* rebalance(struct avl_node*);
...
struct avl_node* rebalance(struct avl_node* N){
    N->height = max(height(N->left), height(N->right)) + 1;
    int bf = getBalance(N); //balance factor
    if (bf < -1){
        int bfLeft = getBalance(N->left); //balance factor left
        if (bfLeft > 0)
        ...
        int bfRight = getBalance(N->right);
        if (bfRight < 0)
...
//         }
//         struct avl_node* new_root = rightRotate(node);
//         if (new_root->parent != NULL){
//             if (new_root->parent->left  == node){
//                 new_root->parent->left = node->parent;
//             }
//             else 
//                 new_root->parent->right = node->parent;
//         }
//     }
//     else if (balance > 1){
...
//         }
//         struct avl_node* new_root = leftRotate(node);
//         if (new_root->parent != NULL){
//             if (new_root->parent->left  == node){
//                 new_root->parent->left = node->parent;
//             }
...
// struct avl_node* rebalance(struct avl_node* node, int key){
